    ...
    const Record& record)
{
    ...
    const auto distance = distanceValue.getRawValueAs<nautilus::val<double>>();
    const auto neighbourId = neighbourValue.getRawValueAs<nautilus::val<std::uint64_t>>();
...
Record KnnAggregationPhysicalFunction::lower(
